...
from tdnn_layer import ReluBatchNormDropoutLayer
...
def get_tdnn_model(feat_dim,
                   output_dim,
                   hidden_dim):
    model = TdnnModel(feat_dim=feat_dim,
                      output_dim=output_dim,
                      hidden_dim=hidden_dim)
    return model
...
class TdnnModel(nn.Module):
    def __init__(self,feat_dim,output_dim,hidden_dim,context_list=[[-1,0,1],[-1,0,1],[-1,0,1],[-3,0,3],[-3,0,3],[-3,0,3]]):
        super().__init__()
        feat_dim = feat_dim * 3 # add detal
        context_list=context_list
        num_layers = len(context_list)
        tdnns=[]
        #logging.info(context_list[0])
        #logging.info(context_list)
        #logging.info(feat_dim*len(context_list[0]))
        #logging.info(output_dim)
        layer_1 = ReluBatchNormDropoutLayer(input_dim=feat_dim,output_dim=hidden_dim,context=context_list[0])
        tdnns.append(layer_1)
        for i in range(1,num_layers):
            context = context_list[i]
            layer = ReluBatchNormDropoutLayer(input_dim=hidden_dim,output_dim=hidden_dim,context=context_list[i])
            tdnns.append(layer)
        self.tdnns = nn.ModuleList(tdnns)

        self.prefinal_chain = ReluBatchNormDropoutLayer(input_dim=hidden_dim,output_dim=hidden_dim,context=[0],stride=3)
        self.output_affine = nn.Linear(in_features = hidden_dim,out_features = output_dim)

        self.prefinal_xent = ReluBatchNormDropoutLayer(input_dim=hidden_dim,output_dim=hidden_dim,context=[0],stride=3)
        self.output_xent_affine = nn.Linear(in_features = hidden_dim,out_features = output_dim)


    def forward(self,x,dropout=0.):
        # input x is of shape: [batch_size, seq_len, feat_dim] = [N, T, C]
        assert x.ndim == 3
        x = x.permute(0, 2, 1) # x is [N,C,T]
        for i in range(len(self.tdnns)):
            x = self.tdnns[i](x,dropout = dropout)
